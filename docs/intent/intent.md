...
> for the path that led to this framing — building personal wikis, handing them to agents as context, and noticing what changed — see [origin.md](./origin.md). this document describes what we are building and why, in terms of the final structure.

## the thesis

the non-determinism of large language models is largely a context selection problem in disguise. with a well-aligned context — a slice of the world that overlaps with the question being asked — a sufficiently capable model produces reliable answers. with a bad context, variance spikes and the model drifts into the wrong region of its distribution.
...
what we are building, then, is two things together: a large, high-quality knowledge base, and a strategy for selecting slices of it as context for specific problems. the rest of this document describes the shape of that knowledge base and the mechanism by which slices are selected.
...
the wiki we are building is therefore large by design. think of it as wikipedia, except the editors are agents rather than humans. they read sources, write entries, discuss, revise, and continuously shape the vault. that editing process is where much of the project's attention goes.
...
the persona problem splits into two parts:
